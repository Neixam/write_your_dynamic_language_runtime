        ...
        for (; ; ) {
            switch (instrs[pc++]) {
                ...
                case Instructions.LOOKUP -> {
                    ...
                    var ret = globalEnv.lookup(name);
                    push(stack, sp++, encodeAnyValue(ret, dict));
                ...
                case Instructions.REGISTER -> {
                    ...
                    String name = (String) TagValues.decodeAnyValue(indexTagValue, dict, heap);
                    // pop the value from the stack and decode it
                    Object value = TagValues.decodeAnyValue(pop(stack, --sp), dict, heap);
...
//                                receiver + " " + java.util.Arrays.toString(args));
                    ...
                    var activation = baseArg + RECEIVER_BASE_ARG_OFFSET + code.slotCount();
                    ...
                    pc = 0;
                    bp = baseArg + RECEIVER_BASE_ARG_OFFSET;
                ...
                case Instructions.RET -> {
                   ...
                   // System.err.println("ret " + decodeAnyValue(result, dict, heap));
                    ...
                    sp = bp + RECEIVER_BASE_ARG_OFFSET;
                ...
                case Instructions.GET -> {
                    ...
                    var fieldName = (String) decodeAnyValue(instrs[pc++], dict, heap);
                    ...
                    // get the field index
                    int fieldIndex = ref + OBJECT_HEADER_SIZE + (Integer) slot;
                    // get field value
                    int fieldValue = heap[fieldIndex];
                ...
                case Instructions.PUT -> {
                    ...
                    // get the field index
                    var fieldIndex = ref + OBJECT_HEADER_SIZE + (Integer) slotOrUndefined;
                    // store field value from the top of the stack on heap
                    heap[fieldIndex] = value;
                }
                case Instructions.PRINT -> {
                    // pop the value on top of the stack
                    var arg = pop(stack, --sp);
                    //var result = ...;
                    // decode the value
                    var value = decodeAnyValue(arg, dict, heap);
